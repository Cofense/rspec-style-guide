...
Use contexts to organize examples that share an initial state. This eases readability and improves maintainability.
A context should perform some test setup. A "transparent" context that performs no setup may be replaceable with a `describe`.

[source,ruby]
----
# bad - contexts not used
it 'has 200 status code if logged in' do
  set_user_logged_in
  expect(response).to respond_with 200
end

it 'shows the welcome message if logged in' do
  set_user_logged_in
...
# bad - the second context performs nothing
...
`context` blocks should cover all possible cases.
It is a code smell if there is a single context. The matching negative case or parallel cases should be covered by additional contexts. If other cases do not exist, the lone context should be merged into the parent block.
...
# good - positive and negative case
describe '#attributes' do
  context 'when the display name is present' do
  end
...
# good - three parallel cases
describe '#title' do
  context 'when the document has been approved' do
  end

  context 'when the document is in review' do
  end

  context 'when the document has been denied' do
  end
end
----
...
Consider using an adjunct phrase starting with 'when', 'with', 'without', or similar words.
